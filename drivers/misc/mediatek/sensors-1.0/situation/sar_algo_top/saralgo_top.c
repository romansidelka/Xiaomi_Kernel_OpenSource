...
static int saralgo_factory_set_step(int32_t* step_en)
{
	...
	err = sensor_cfg_to_hub(ID_SAR_ALGO_TOP,(uint8_t *)cfg_data,count);
	if (err < 0){
		pr_err_ratelimited("saralgo_get_data_from_hub fail!!\n");
		return -1;
	}
...
static int saralgo_set_step(uint8_t *data, uint8_t count)
{
	...
	err = sensor_cfg_to_hub(ID_SAR_ALGO_TOP,(uint8_t *)cfg_data,count_size);
	if (err < 0){
		pr_err_ratelimited("saralgo_get_data_from_hub fail!!\n");
		return -1;
	}
