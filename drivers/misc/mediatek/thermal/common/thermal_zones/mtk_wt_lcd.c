...
#define mtkwt_lcd_dprintk(fmt, args...)   \
pr_err("[Thermal/WT/LCD]" fmt, ##args)

#define mtkwt_lcd_printk(fmt, args...) \
pr_err("[Thermal/WT/LCD]" fmt, ##args)
struct WTLCD_TEMPERATURE {
	__s32 WTLCD_Temp;
	__s32 TemperatureR;
};
#if  IS_ENABLED(CONFIG_MEDIATEK_MT6577_AUXADC)
...
static int g_TAP_over_critical_low = BTLCD_TAP_OVER_CRITICAL_LOW;
static int g_RAP_pull_up_voltage = BTLCD_RAP_PULL_UP_VOLTAGE;
static int g_RAP_ntc_table = BTLCD_RAP_NTC_TABLE;
//static int g_RAP_ADC_channel = BTLCD_RAP_ADC_CHANNEL;
static int g_btsctherm_TemperatureR;
static struct WTLCD_TEMPERATURE *Temperature_Table;
static int ntc_tbl_size;
/* NCP15WF104F03RC(100K) */
static struct WTLCD_TEMPERATURE WTLCD_Temperature_Table[] = {
	{-40, 4397119},
	{-35, 3088599},
	{-30, 2197225},
	{-25, 1581881},
...
void mtkwt_lcd_prepare_table(int table_num)
{
	/* NCP15WF104F03RC */
	Temperature_Table = WTLCD_Temperature_Table;
	ntc_tbl_size = sizeof(WTLCD_Temperature_Table);
	pr_notice("[Thermal/WT/LCD] %s table_num=%d\n", __func__, table_num);
}
/* convert register to temperature  */
static __s32 mtkwt_lcd_thermistor_conver_temp(__s32 Res)
{
	int i = 0;
	int asize = 0;
	__s32 RES1 = 0, RES2 = 0;
	__s32 TAP_Value = -200, TMP1 = 0, TMP2 = 0;
#ifdef APPLY_PRECISE_LCD_TEMP
	TAP_Value = TAP_Value * 1000;
#endif
	asize = (ntc_tbl_size / sizeof(struct WTLCD_TEMPERATURE));
...
static __s32 mtk_wt_lcd_volt_to_temp(__u32 dwVolt)
{
	__s32 TRes;
	__u64 dwVCriAP = 0;
	__s32 WTLCD_TMP = -100;
	...
	/* convert register to temperature */
	WTLCD_TMP = mtkwt_lcd_thermistor_conver_temp(TRes);
	return WTLCD_TMP;
}
static int get_hw_wtlcd_temp(void)
{
#if  IS_ENABLED(CONFIG_MEDIATEK_MT6577_AUXADC)
	int val = 0;
	int ret = 0, output;
...
#endif
#if  IS_ENABLED(CONFIG_MEDIATEK_MT6577_AUXADC)
	ret = iio_read_channel_processed(thermistor_ch7, &val);
	if (ret < 0) {
		mtkwt_lcd_printk("Busy/Timeout, IIO ch read failed %d\n", ret);
		return ret;
	}
#ifdef APPLY_PRECISE_LCD_TEMP
	ret = val * 100;
	...
	mtkwt_lcd_dprintk("APtery output mV = %d\n", ret);
	output = mtk_wt_lcd_volt_to_temp(ret);
	mtkwt_lcd_dprintk("lcd output temperature = %d\n", output);
	return output;
}
static DEFINE_MUTEX(WTLCD_lock);
/*int ts_AP_at_boot_time = 0;*/
int mtkwt_lcd_get_hw_temp(void)
{
	int t_ret = 0;
	int t_ret2 = 0;
	mutex_lock(&WTLCD_lock);
	t_ret = get_hw_wtlcd_temp();
#ifndef APPLY_PRECISE_LCD_TEMP
	t_ret = t_ret * 1000;
#endif
	mutex_unlock(&WTLCD_lock);
	if ((tsatm_thermal_get_catm_type() == 2) &&
		(tsdctm_thermal_get_ttj_on() == 0))
		t_ret2 = wakeup_ta_algo(TA_CATMPLUS_TTJ);
	if (t_ret2 < 0)
		pr_notice("[Thermal/WT/LCD]wakeup_ta_algo out of memory\n");
	if (t_ret > 40000)	/* abnormal high temp */
		mtkwt_lcd_printk("T_AP=%d\n", t_ret);
	mtkwt_lcd_dprintk("[%s] T_AP, %d\n", __func__, t_ret);
	return t_ret;
}
static int mtkwt_lcd_get_temp(struct thermal_zone_device *thermal, int *t)
{
	*t = mtkwt_lcd_get_hw_temp();
...
static struct thermal_zone_device_ops mtkwt_LCD_dev_ops = {
	.get_temp = mtkwt_lcd_get_temp,
};
static void mtkwt_lcd_unregister_thermal(void)
{
	mtkwt_lcd_dprintk("[%s]\n", __func__);
	if (thz_dev) {
...
static int mtkwt_lcd_register_thermal(void)
{
	mtkwt_lcd_dprintk("[%s]\n", __func__);
	/* trips : trip 0~1 */
	thz_dev = mtk_thermal_zone_device_register("backlight_therm", num_trip, NULL,
						&mtkwt_LCD_dev_ops, 0, 0, 0,
						interval * 1000);
	return 0;
}

static int mtkwt_lcd_probe(struct platform_device *pdev)
{
	int err = 0;
	int ret = 0;
	...
	mtkwt_lcd_dprintk("[%s]\n", __func__);
	if (!pdev->dev.of_node) {
		mtkwt_lcd_printk("[%s] Only DT based supported\n",
			__func__);
		return -ENODEV;
	}

	thermistor_ch7 = devm_kzalloc(&pdev->dev, sizeof(*thermistor_ch7),
		GFP_KERNEL);
	if (!thermistor_ch7)
		return -ENOMEM;

	thermistor_ch7 = iio_channel_get(&pdev->dev, "thermistor-ch7");
	ret = IS_ERR(thermistor_ch7);
	if (ret) {
		mtkwt_lcd_printk("[%s] fail to get auxadc iio ch7: %d\n",
			__func__, ret);
		return ret;
	}

	/* setup default table */
	mtkwt_lcd_prepare_table(g_RAP_ntc_table);
	mtkwt_lcd_register_thermal();
	return err;
}
#ifdef CONFIG_OF
...
static struct platform_driver mtk_thermal_wtlcd_driver = {
	.remove = NULL,
	.shutdown = NULL,
	.probe = mtkwt_lcd_probe,
	.suspend = NULL,
	.resume = NULL,
	.driver = {
...
int mtkwt_lcd_init(void)
{
	int err = 0;
	mtkwt_lcd_dprintk("[%s]\n", __func__);
#if  IS_ENABLED(CONFIG_MEDIATEK_MT6577_AUXADC)
	err = platform_driver_register(&mtk_thermal_wtlcd_driver);
	if (err) {
		mtkwt_lcd_printk("thermal driver callback register failed.\n");
		return err;
	}
#else
	/* setup default table */
...
void mtkwt_lcd_exit(void)
{
	mtkwt_lcd_dprintk("[%s]\n", __func__);
	mtkwt_lcd_unregister_thermal();
