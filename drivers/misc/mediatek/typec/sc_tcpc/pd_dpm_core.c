...
extern int tcpci_is_sy6976(struct tcpc_device *tcpc);
		...
		struct pd_port_power_caps *src_cap, uint8_t charging_policy)
{
	...
	uint8_t last_src_nr = 0;
	struct dpm_pdo_info_t sink, source;

	if (tcpci_is_sy6976(pd_port->tcpc)) {
		DPM_DBG("dpm_build_request_info_with_new_src_cap:is_sy6976\n");
		last_src_nr = pd_port->pe_data.last_cap_number;
		if (sel > src_cap->nr || (last_src_nr != src_cap->nr))
			return false;
        } else {
		if (sel > src_cap->nr)
			return false;
		...
		struct pd_port_power_caps *src_cap, uint8_t charging_policy)
{
	...
	if (tcpci_is_sy6976(tcpc)) {
		DPM_DBG("dpm_build_request_info_pdo:is_sy6976\n");
		pd_port->pe_data.last_cap_number = src_cap->nr;
        }
	return max_uw > 0;
}
...
EXPORT_SYMBOL(pd_dpm_inform_country_info);
...
EXPORT_SYMBOL(pd_dpm_send_country_info);
