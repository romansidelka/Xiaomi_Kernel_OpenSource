...
int usb_typec_mode = NON_COMPLIANT;
extern int chg_set_typec_mode(int mode);
				...
				unsigned long event, void *data)
{
	...
	case TCP_NOTIFY_SOURCE_VBUS:
		...
		if (noti->vbus_state.mv) {
			val.intval = 1;
#if IS_ENABLED (CONFIG_SOUTHCHIP_CHARGER_CLASS)
			charger_set_otg(rpmd->charger, true);
#endif
		} else {
			val.intval = 0;
#if IS_ENABLED (CONFIG_SOUTHCHIP_CHARGER_CLASS)
			charger_set_otg(rpmd->charger, false);
#endif
		}
	...
	case TCP_NOTIFY_TYPEC_STATE:
		...
		if (old_state == TYPEC_UNATTACHED &&
			...
			usb_typec_mode = SOURCE_ATTACHED;
#if IS_ENABLED(CONFIG_XIAOMI_USB_TOUCH_NOTIFIER)
			set_plug_status(XIAOMI_USB_ENABLE);
			xiaomi_touch_usb_data.usb_touch_enable = XIAOMI_USB_ENABLE;
			xiaomi_usb_touch_notifier_call_chain(XIAOMI_TOUCH_USB_SWITCH,
				&xiaomi_touch_usb_data);
#endif
			...
			usb_typec_mode = NOTHING_ATTACHED;
			cancel_delayed_work_sync(&rpmd->usb_dwork);
			rpmd->usb_dr = DR_IDLE;
			schedule_delayed_work(&rpmd->usb_dwork, 0);
#if IS_ENABLED(CONFIG_XIAOMI_USB_TOUCH_NOTIFIER)
			set_plug_status(XIAOMI_USB_DISABLE);
		...
		chg_set_typec_mode(usb_typec_mode);
#if 0//(LINUX_VERSION_CODE >= KERNEL_VERSION(5, 4, 0))
...
#if IS_ENABLED (CONFIG_SOUTHCHIP_CHARGER_CLASS)
	rpmd->charger = charger_find_dev_by_name("sc_charger");
	if (!rpmd->charger) {
		dev_err(rpmd->dev, "%s get sc_charger fail\n", __func__);
		ret = -EPROBE_DEFER;
		goto err_get_sc_charger;
	}
#endif
