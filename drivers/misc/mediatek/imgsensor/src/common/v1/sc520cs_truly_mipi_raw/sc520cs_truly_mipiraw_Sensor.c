...
#include "sc520cs_truly_mipiraw_Sensor.h"

#define PFX "sc_sc520cs_camera_sensor"
...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = SC520CS_TRULY_MIPI_RAW_SENSOR_ID,
...
static void set_shutter(kal_uint32 shutter)
{
	...
	LOG_INF("set_shutter shutter = %d", shutter);
...
static void sensor_init(void)
{
    ...
    write_cmos_sensor8(0x37fa,0x17);
    write_cmos_sensor8(0x37fb,0x64);
    write_cmos_sensor8(0x37fc,0x10);
    write_cmos_sensor8(0x37fd,0x08);
    ...
    write_cmos_sensor8(0x320d,0xd7);
    ...
    write_cmos_sensor8(0x4405,0x23);
    write_cmos_sensor8(0x4407,0x0c);
    write_cmos_sensor8(0x440c,0x2c);
    write_cmos_sensor8(0x440d,0x2c);
    write_cmos_sensor8(0x440e,0x21);
    write_cmos_sensor8(0x440f,0x37);
...
static void sc520cs_fusion_id_read(void)
{
	...
	for (i=0; i<11; i++) {
		...
		pr_debug("%s %d wpc fusion_id_front[%d]=0x%2x\n",__func__, __LINE__, i, fusion_id_front[i]);
	}
}
static void sc520cs_sn_id_read(void)
{
	...
	for (i=0; i<14; i++) {
		...
		pr_debug("%s %d wpc sn_front[%d]=0x%2x\n",__func__, __LINE__, i, sn_front[i]);
	}
}
#if 0
static void get_sensor_module_info(void)
{
    ...
    for (i = 0; i < 15; i++) {
        ...
        pr_debug("%s %d sc520cs module_id[%d]=0x%2x\n",__func__, __LINE__, i, calmoduleversion[i]);
    }
    pr_debug("sc520cs======================module version==================\n");
    pr_debug("sc520cs[vendor id] = 0x%x\n", calmoduleversion[0]);
    pr_debug("sc520cs[yy/mm/dd] = %d/%d/%d \n", calmoduleversion[1], calmoduleversion[2], calmoduleversion[3]);
    pr_debug("sc520cs[hh/mm/sec] = %d:%d:%d \n", calmoduleversion[4], calmoduleversion[5], calmoduleversion[6]);
    pr_debug("sc520cs[lens_id/vcmid/drive id] = 0x%x/0x%x/0x%x \n", calmoduleversion[7], calmoduleversion[8], calmoduleversion[9]);
    pr_debug("sc520cs======================module version==================\n");
}
#endif
	...
	case SENSOR_FEATURE_GET_PIXEL_CLOCK_FREQ_BY_SCENARIO:
		...
		case MSDK_SCENARIO_ID_SLIM_VIDEO:
			...
			*feature_para_len = 4;
			break;
		case MSDK_SCENARIO_ID_CUSTOM1:
			*feature_return_para_32 = imgsensor_info.custom1.pclk;
			*feature_para_len = 4;
			break;
	    case MSDK_SCENARIO_ID_CUSTOM2:
			*feature_return_para_32 = imgsensor_info.custom2.pclk;
			*feature_para_len = 4;
			break;
	    case MSDK_SCENARIO_ID_CUSTOM3:
			*feature_return_para_32 = imgsensor_info.custom3.pclk;
			*feature_para_len = 4;
			break;
		case MSDK_SCENARIO_ID_CAMERA_PREVIEW:
		default:
	...
	case SENSOR_FEATURE_GET_PERIOD_BY_SCENARIO:
		...
		case MSDK_SCENARIO_ID_SLIM_VIDEO:
			...
			*feature_para_len = 4;
			break;
		case MSDK_SCENARIO_ID_CUSTOM1:
			*feature_return_para_16++ = imgsensor_info.custom1.linelength;//imgsensor.line_length;
			*feature_return_para_16 = imgsensor_info.custom1.framelength;//imgsensor.frame_length;
			*feature_para_len = 4;
			break;
		case MSDK_SCENARIO_ID_CUSTOM2:
			*feature_return_para_16++ = imgsensor_info.custom2.linelength;//imgsensor.line_length;
			*feature_return_para_16 = imgsensor_info.custom2.framelength;//imgsensor.frame_length;
			*feature_para_len = 4;
			break;
		case MSDK_SCENARIO_ID_CUSTOM3:
