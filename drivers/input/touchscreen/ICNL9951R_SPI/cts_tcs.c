...
#define CMD_SYS_STS_GAME_THRESHOLD_MODE_RW        	(0x625f)
#define CMD_SYS_STS_GAME_TOLERANCE_MODE_RW        	(0x6260)
#define CMD_SYS_STS_GAME_EDGE_INHIBITION_MODE_RW   (0x6261)
...
{
	cts_info("Set panel direction:%d", direction);
...
int cts_tcs_set_game_threshold_value(struct cts_device *cts_dev, u8 value)
{
    ...
    cts_info("Set game threshold value:%d", value);
    ret = cts_tcs_write(cts_dev, CMD_SYS_STS_GAME_THRESHOLD_MODE_RW,
            &value, sizeof(value));
    if (ret) {
        cts_err("Set game threshold value failed!");
    }
...
int cts_tcs_set_game_tolerance_value(struct cts_device *cts_dev, u8 value)
{
    ...
    cts_info("Set game tolerance value:%d", value);
    ret = cts_tcs_write(cts_dev, CMD_SYS_STS_GAME_TOLERANCE_MODE_RW,
            &value, sizeof(value));
    if (ret) {
        cts_err("Set game tolerance value failed!");
    }
...
int cts_tcs_set_game_edge_inhibition_value(struct cts_device *cts_dev, u8 value)
{
    ...
    cts_info("Set game edge inhibition value:%d", value);
    ret = cts_tcs_write(cts_dev, CMD_SYS_STS_GAME_EDGE_INHIBITION_MODE_RW,
            &value, sizeof(value));
    if (ret) {
        cts_err("Set game edge inhibition value failed!");
    }
...
int cts_tcs_set_proximity_mode(struct cts_device *cts_dev, u8 enable)
{
    ...
    if(enable){
		 cts_dev->rtdata.proximity_status = 0;
		 cts_info("Default far event while resuming");
	}
