...
bool icnl_90hz_proximity_status = false;
void panel_90hz_check_proximity_mode(bool proximity_mode)
{
	icnl_90hz_proximity_status = proximity_mode;
}
EXPORT_SYMBOL_GPL(panel_90hz_check_proximity_mode);

bool icnl_90hz_gesture_status = false;
void panel_90hz_check_gesture_mode(bool gst_mode)
{
	icnl_90hz_gesture_status = gst_mode;
}
EXPORT_SYMBOL_GPL(panel_90hz_check_gesture_mode);
...
static int lcm_unprepare(struct drm_panel *panel)
{
	...
	if (icnl_90hz_proximity_status == true) {
...
static int lcm_probe(struct mipi_dsi_device *dsi)
{
	...
	if (lcm_name) {
		...
		if (strcmp("dsi_panel_n85_46_0f_0c_90_vdo_lcm_drv",videolfb_tag->lcmname) == 0 ){
			pr_info(" find lcm: %s \n", "unknow");
		}
		else {
...
static struct mipi_dsi_driver lcm_driver = {
	...
	.remove = lcm_remove,
	.driver = {
		.name = "dsi_panel_n85_46_0f_0c_90_vdo",
		.owner = THIS_MODULE,
