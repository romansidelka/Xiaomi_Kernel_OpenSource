...
{
	int chg_status = 0;
	charger->ops->get_chg_status1(charger, &chg_status);
	return  chg_status;
}
EXPORT_SYMBOL(charger_get_chg_status1);
										...
										uint32_t *chg_status)
{
	...
	if (charger->ops->get_chg_status == NULL)
		return -EOPNOTSUPP;
	return charger->ops->get_chg_status(charger, chg_status);
}
...
int charger_get_chg_en(struct charger_dev * charger, bool *en)
{
    if (!charger || !charger->ops)
        return -EINVAL;
    if (charger->ops->get_chg_en == NULL)
        return -EOPNOTSUPP;
    return charger->ops->get_chg_en(charger, en);
}
...
int charger_get_ichg(struct charger_dev * charger)
{
    ...
    if (charger->ops->get_ichg == NULL)
        return -EOPNOTSUPP;
    return charger->ops->get_ichg(charger);
}
...
int charger_set_safety_time_en(struct charger_dev *charger, bool en)
{
    if (!charger || !charger->ops)
        return -EINVAL;
    if (charger->ops->set_safety_time_en == NULL)
        return -EOPNOTSUPP;
    return charger->ops->set_safety_time_en(charger, en);
}
...
int charger_is_safety_time_en(struct charger_dev *charger, bool *en)
{
    if (!charger || !charger->ops)
        return -EINVAL;
    if (charger->ops->is_safety_time_en == NULL)
        return -EOPNOTSUPP;
    return charger->ops->is_safety_time_en(charger, en);
}
...
int charger_qc_identify(struct charger_dev * charger)
{
	...
	if (charger->ops->qc_identify == NULL)
		return -EOPNOTSUPP;
	return charger->ops->qc_identify(charger);
}
...
int charger_is_sy6976(struct charger_dev * charger)
{
	if (!charger || !charger->ops)
		return -EINVAL;
	if (charger->ops->is_sy6976 == NULL)
		return -EOPNOTSUPP;
	return charger->ops->is_sy6976(charger);
}
EXPORT_SYMBOL(charger_is_sy6976);
...
MODULE_DESCRIPTION("Southchip Charger Class Core");
